...
export default function MessagesWidget({ accentColor = "#F47B20" }: Props) {
  ...
  return (
    <>
          ...
          {!activeConv ? (
            ...
            <div className="chat-view">
              ...
              <form className="chat-input" onSubmit={sendMessage}>
                <input placeholder="Type..." value={newMsg} onChange={(e) => setNewMsg(e.target.value)} />
                        ...
                        {userResults.map((u) => (
                          <div key={u.userId} className={`nc-user ${selectedUser?.userId===u.userId?"selected":""}`} onClick={() => { setSelectedUser(u); setUserSearch(u.fullName); setUserResults([]); }}>
                            ...
                            <div className="nu-info">
                              ...
                              <div className="nu-role">{u.role?.replace(/_/g," ")} - {u.email}</div>
                            </div>
                          </div>
                        ))}
                      </div>
      ...
      <style>{`
        .msg-fab {
          position:fixed; bottom:80px; right:1.25rem; z-index:9999 !important;
          background:var(--accent,#F47B20); color:#fff; border:none;
          border-radius:50%; width:48px; height:48px;
          font-family:var(--font-display); font-size:0.65rem; font-weight:700;
          letter-spacing:0.06em; cursor:pointer; box-shadow:0 4px 16px rgba(0,0,0,0.2);
          display:flex; align-items:center; justify-content:center; transition:all 0.2s; position:fixed;
        }
        ...
        .fab-badge {
          ...
          justify-content:center; font-size:0.6rem; font-weight:700; border:2px solid #fff;
        }
        .msg-panel {
          position:fixed; bottom:140px; right:1.25rem; z-index:9998 !important;
          width:320px; height:480px; background:#fff; border-radius:16px;
          box-shadow:0 16px 48px rgba(0,0,0,0.18); border:1.5px solid #E5E5E5;
          display:flex; flex-direction:column; overflow:hidden;
        }
        .mp-header {
          display:flex; align-items:center; gap:0.5rem;
          padding:0.875rem 1rem; border-bottom:1px solid #E5E5E5;
          background:var(--accent,#F47B20); color:#fff; flex-shrink:0;
        }
        ...
        .conv-empty { display:flex; flex-direction:column; align-items:center; justify-content:center; gap:0.75rem; height:100%; color:#A3A3A3; font-size:0.875rem; }
        ...
        .chat-msgs { flex:1; overflow-y:auto; padding:0.875rem; display:flex; flex-direction:column; gap:0.5rem; }
        .msg-row { display:flex; }
        .msg-row.me { justify-content:flex-end; }
        .msg-bubble { max-width:80%; padding:0.5rem 0.75rem; border-radius:10px; display:flex; flex-direction:column; gap:0.15rem; }
        ...
        .msg-text { font-size:0.825rem; line-height:1.4; word-break:break-word; }
        ...
        .chat-input input { flex:1; background:#F5F5F5; border:1.5px solid #E5E5E5; border-radius:20px; padding:0.5rem 0.875rem; color:#171717; font-size:0.825rem; font-family:var(--font-body); outline:none; transition:border-color 0.2s; }
        ...
        .nc-input { width:100%; background:#F5F5F5; border:1.5px solid #E5E5E5; border-radius:6px; padding:0.65rem 0.875rem; color:#171717; font-size:0.825rem; font-family:var(--font-body); outline:none; transition:border-color 0.2s; }
        ...
        .nc-user { display:flex; align-items:center; gap:0.625rem; padding:0.65rem 0.875rem; cursor:pointer; border-bottom:1px solid #F5F5F5; transition:background 0.15s; }
        ...
        .nu-avatar img { width:100%; height:100%; object-fit:cover; }
