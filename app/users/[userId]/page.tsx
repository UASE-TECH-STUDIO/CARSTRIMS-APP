  ...
  return (
    <div style={{minHeight:"100vh",background:"#F5F5F5",fontFamily:"var(--font-body)"}}>
      ...
      <div style={{background:"#fff",borderBottom:"1.5px solid #E5E5E5"}}>
        ...
        <div style={{display:"flex",alignItems:"flex-end",gap:"1.25rem",padding:"0 1.5rem 1.5rem",marginTop:"-62px",flexWrap:"wrap"}}>
          <div
            ...
            style={{width:"120px",height:"120px",borderRadius:"50%",overflow:"hidden",border:"5px solid #fff",background:"#FFF7ED",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"var(--font-display)",fontSize:"3rem",color:rc,flexShrink:0,cursor:profile.avatar?"zoom-in":"default",boxShadow:"0 4px 20px rgba(0,0,0,0.15)",transition:"transform 0.2s"}}>
            {profile.avatar
          ...
          <div style={{paddingTop:"0.875rem",display:"flex",flexDirection:"column",gap:"0.4rem",flex:1,minWidth:0}}>
            ...
            {(profile.city||profile.state) && (
              <div style={{fontSize:"0.875rem",color:"#737373",display:"flex",alignItems:"center",gap:"0.25rem"}}>
      ...
      <div style={{maxWidth:"820px",margin:"0 auto",padding:"1.5rem",display:"flex",flexDirection:"column",gap:"1rem"}}>
        ...
        {/* Message button (mobile prominent) */}
        {!isSelf && isAuthenticated && (
          <button onClick={handleMessage} disabled={startingMsg}
            style={{background:"#1A1A1A",color:"#fff",border:"none",borderRadius:"12px",padding:"1rem",fontFamily:"var(--font-display)",fontSize:"1rem",letterSpacing:"0.1em",cursor:"pointer",opacity:startingMsg?0.6:1,display:"block",width:"100%",transition:"background 0.2s"}}
        ...
        {(profile.phone||profile.whatsapp||profile.email) && (
          <div style={{background:"#fff",border:"1.5px solid #E5E5E5",borderRadius:"14px",overflow:"hidden"}}>
            <div style={{fontSize:"0.68rem",fontWeight:800,letterSpacing:"0.18em",textTransform:"uppercase",color:"#A3A3A3",padding:"0.875rem 1.25rem",borderBottom:"1px solid #E5E5E5",background:"#FAFAFA"}}>Contact</div>
            <div style={{display:"flex",flexDirection:"column",gap:"0.5rem",padding:"1.25rem"}}>
              {profile.phone && <a href={`tel:${profile.phone}`} style={{textDecoration:"none",padding:"0.875rem 1rem",borderRadius:"10px",fontSize:"0.95rem",display:"flex",alignItems:"center",gap:"0.625rem",background:"#EFF6FF",color:"#3B8BD4",border:"1.5px solid rgba(59,139,212,0.25)",fontWeight:600,transition:"all 0.2s"}}>📞 {profile.phone}</a>}
              {profile.whatsapp && <a href={`https://wa.me/${profile.whatsapp}`} target="_blank" rel="noreferrer" style={{textDecoration:"none",padding:"0.875rem 1rem",borderRadius:"10px",fontSize:"0.95rem",display:"flex",alignItems:"center",gap:"0.625rem",background:"#F0FDF4",color:"#16A34A",border:"1.5px solid rgba(22,163,74,0.25)",fontWeight:600,transition:"all 0.2s"}}>💬 WhatsApp</a>}
              {profile.email && <a href={`mailto:${profile.email}`} style={{textDecoration:"none",padding:"0.875rem 1rem",borderRadius:"10px",fontSize:"0.95rem",display:"flex",alignItems:"center",gap:"0.625rem",background:"#FFF7ED",color:"#F47B20",border:"1.5px solid rgba(244,123,32,0.25)",fontWeight:600,transition:"all 0.2s"}}>✉ {profile.email}</a>}
            </div>
          </div>
        )}
        ...
        {(profile.instagram||profile.facebook||profile.twitter||profile.tiktok||profile.website) && (
          <div style={{background:"#fff",border:"1.5px solid #E5E5E5",borderRadius:"14px",overflow:"hidden"}}>
            <div style={{fontSize:"0.68rem",fontWeight:800,letterSpacing:"0.18em",textTransform:"uppercase",color:"#A3A3A3",padding:"0.875rem 1.25rem",borderBottom:"1px solid #E5E5E5",background:"#FAFAFA"}}>Social & Web</div>
        ...
        {/* Dealer card — for DEALER_ADMIN and DEALER_STAFF */}
        {isDealer && profile.dealer && (
          <div style={{background:"#fff",border:"1.5px solid #E5E5E5",borderRadius:"14px",overflow:"hidden"}}>
            <div style={{fontSize:"0.68rem",fontWeight:800,letterSpacing:"0.18em",textTransform:"uppercase",color:"#A3A3A3",padding:"0.875rem 1.25rem",borderBottom:"1px solid #E5E5E5",background:"#FAFAFA"}}>
            ...
            <Link href={`/dealers/${profile.dealer.dealerId}`}
              ...
              <div style={{width:"64px",height:"64px",borderRadius:"12px",overflow:"hidden",background:"#FFF7ED",border:"1.5px solid rgba(244,123,32,0.3)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"var(--font-display)",fontSize:"1.5rem",color:"#F47B20",flexShrink:0}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:"0.82rem",color:"#F47B20",marginTop:"0.3rem",fontWeight:600}}>View dealer profile & all cars →</div>
              </div>
            </Link>
          </div>
        )}
        ...
        {isPartner && profile.stats && (
          <div style={{background:"#fff",border:"1.5px solid #E5E5E5",borderRadius:"14px",overflow:"hidden"}}>
            <div style={{fontSize:"0.68rem",fontWeight:800,letterSpacing:"0.18em",textTransform:"uppercase",color:"#A3A3A3",padding:"0.875rem 1.25rem",borderBottom:"1px solid #E5E5E5",background:"#FAFAFA"}}>Partner Activity</div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(120px,1fr))",gap:"1rem",padding:"1.25rem"}}>
              {[["Cars Assigned",profile.stats.totalCars||0],["Dealers",profile.stats.totalDealers||0]].map(([l,v])=>(
                <div key={l} style={{background:"#F5F5F5",borderRadius:"10px",padding:"1rem",textAlign:"center"}}>
                  <div style={{fontFamily:"var(--font-display)",fontSize:"2rem",color:"#F47B20"}}>{v}</div>
                  <div style={{fontSize:"0.72rem",color:"#737373",marginTop:"0.25rem"}}>{l}</div>
                </div>
              ))}
            </div>
          </div>
