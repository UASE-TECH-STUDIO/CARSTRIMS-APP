...
const ROLES = [
  { value:"DEALER_ADMIN", label:"Dealer / Car Stand", desc:"Manage inventory, staff and sales from your dealership" },
...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState({ fullName:"", username:"", email:"", password:"", phone:"", whatsapp:"", address:"" });
  ...
  if (done) return (
    <div style={{minHeight:"100vh",background:"#F5F5F5",display:"flex",alignItems:"center",justifyContent:"center",padding:"2rem",fontFamily:"var(--font-body)"}}>
      <div style={{maxWidth:"480px",width:"100%",background:"#fff",borderRadius:"16px",padding:"2.5rem",boxShadow:"0 4px 24px rgba(0,0,0,0.08)",display:"flex",flexDirection:"column",gap:"1.5rem",alignItems:"center",textAlign:"center"}}>
        ...
        <div>
          <h2 style={{fontFamily:"var(--font-display)",fontSize:"1.6rem",color:"#1A1A1A"}}>Account Created!</h2>
          <p style={{fontSize:"0.9rem",color:"#737373",marginTop:"0.5rem",lineHeight:"1.7"}}>
            {role==="DEALER_ADMIN"
              ? "Sign in and complete your dealership profile setup to get started."
        ...
        {role==="DEALER_ADMIN" && (
          <div style={{background:"#FFF7ED",border:"1px solid rgba(244,123,32,0.3)",borderRadius:"10px",padding:"1.25rem",width:"100%",textAlign:"left",fontSize:"0.85rem",color:"#C4621A",lineHeight:1.8}}>
            ...
            1. Complete your dealership profile (name, address, contacts)<br/>
            2. Access your full dealer dashboard immediately<br/>
            3. Wait for admin approval — your listings are hidden until then
          </div>
        )}
        <button onClick={() => router.push("/login")}
          style={{background:"#F47B20",color:"#fff",border:"none",borderRadius:"8px",padding:"0.875rem 2rem",fontFamily:"var(--font-display)",fontSize:"0.95rem",letterSpacing:"0.12em",cursor:"pointer",width:"100%"}}>
          SIGN IN NOW
        </button>
      </div>
    </div>
  ...
  return (
    <div className="rg-root">
        ...
        <div className="rg-card">
          ...
          {step===1 ? (
            ...
            <form onSubmit={submit} className="rg-form">
              ...
              <div className="rg-field"><label className="rg-lbl">Password *</label><input type="password" className="rg-input" placeholder="Min 8 characters" value={form.password} onChange={(e)=>setForm({...form,password:e.target.value})} required minLength={8}/></div>
              <div className="rg-row">
              ...
              {role==="DEALER_ADMIN"&&<div className="rg-notice"><strong>Dealer Account:</strong> After signing in you complete your dealership setup. Listings are hidden until admin approves your account.</div>}
              <div className="rg-actions">
                <button type="button" className="rg-back" onClick={()=>setStep(1)}>Back</button>
                <button type="submit" className="rg-btn rg-flex1" disabled={loading}>{loading?"Creating...":"CREATE ACCOUNT"}</button>
              </div>
            </form>
          )}
      ...
      <style>{`
        ...
        .rg-foot{font-size:0.7rem;color:#A3A3A3}.rg-foot strong{color:#F47B20}
        ...
        .rg-role-body{flex:1}.rg-role-label{font-size:0.9rem;font-weight:600;color:#1A1A1A}
        ...
        .rg-back{background:#F5F5F5;border:1.5px solid #E5E5E5;color:#525252;border-radius:8px;padding:0.875rem 1.25rem;font-family:var(--font-body);font-size:0.875rem;cursor:pointer}
        ...
        .rg-flex1{flex:1}.rg-switch{font-size:0.875rem;color:#737373;text-align:center}
