...
export default function DealerCarsPage() {
  ...
  const ensureCarSaved = async (): Promise<string|null> => {
    ...
    } catch(ex:any) { setErr(ex.response?.data?.detail || "Save car info first before uploading"); return null; }
  ...
  return (
    <>
      {/* Modals at the TOP — never inside any other component */}
      {markSoldCar && (
        <MarkSoldModal
          car={markSoldCar}
      ...
      <div className="cars-page">
        ...
        <div className="page-header">
          <div>
            ...
            <p className="page-sub">{total} total vehicles</p>
          </div>
        ...
        <div className="filters">
          <input className="search-input" placeholder="Search brand, model, ID..." value={search} onChange={e=>setSearch(e.target.value)} />
          ...
          <div className="cars-grid">
            {cars.map(car => (
              <div key={car._id} className="car-card">
                {/* Image */}
                <div className="car-img" onClick={() => car.images?.[0] && setPreview({src:car.images[0],type:"image"})}>
                  {car.images?.[0]
                    ? <img src={car.images[0]} alt="" />
                    : <div className="car-no-img">🚗</div>
                  }
                  <div className="car-status-badge" style={{background:STATUS_C[car.status]||"#737373"}}>
                    {car.status?.replace(/_/g," ")}
                  </div>
                  {(car.images?.length||0) > 1 && (
                    <div className="car-photo-count">+{car.images.length-1}</div>
                  )}
                </div>
                ...
                <div className="car-actions">
                  <button className="act-btn" onClick={() => openEdit(car)}>✏ Edit</button>
                  {car.status !== "sold" && (
                    <button className="act-btn sold-btn" onClick={() => setMarkSoldCar(car)}>💳 Sold</button>
                  )}
                  <button className="act-btn report-btn" onClick={() => setReportCarId(car.carId)}>📊</button>
                  <button className="act-btn del-btn" onClick={() => handleDelete(car.carId)}>🗑</button>
                </div>
              </div>
            ))}
          </div>
            ...
            <div className="modal" onClick={e => e.stopPropagation()}>
              <div className="modal-header">
                <div>
                  ...
                  {savedCarId && modal==="add" && <div className="draft-note">✓ Draft saved ({savedCarId})</div>}
                </div>
                <button className="modal-close" onClick={closeModal}>✕</button>
              </div>

              <div className="modal-body">
                {err && <div className="form-error"><span>{err}</span><button onClick={()=>setErr("")}>✕</button></div>}

                {/* Photos */}
                <div className="field">
                  <label className="fl">Photos ({images.length}/{MAX_IMAGES})</label>
                  {modal==="add" && !savedCarId && (
                    <div className="upload-hint">💡 Fill in model and price below first, then add photos</div>
                  )}
                  <div className="photo-grid">
                    {images.map((img,i) => (
                      <div key={i} className="photo-thumb" onClick={() => setPreview({src:img,type:"image"})}>
                        <img src={img} alt="" />
                        <button className="photo-del" onClick={async e => {
                          e.stopPropagation();
                          const carId = savedCarId||editCar?.carId;
                          if (carId) { try { await api.delete(`/api/v1/upload/car/${carId}/images`, {data:{image_url:img}}); } catch {} }
                          setImages(p=>p.filter((_,j)=>j!==i));
                        }}>✕</button>
                      </div>
                    ))}
                    {images.length < MAX_IMAGES && (
                      <button className="photo-add" onClick={() => imgInputRef.current?.click()} disabled={uploading}>
                ...
                {/* Video */}
                <div className="field">
                  <label className="fl">Video (optional, max 100MB)</label>
                  {video ? (
                    <div className="video-preview">
                      <div className="video-thumb" onClick={() => setPreview({src:video,type:"video"})}>
                        <video src={video} />
                        <span className="play-icon">▶</span>
                      </div>
                      <div className="video-actions">
                        <button className="act-btn" onClick={() => setPreview({src:video,type:"video"})}>Preview</button>
                        <button className="act-btn del-btn" onClick={async () => {
                          const carId = savedCarId||editCar?.carId;
                          if (carId) { try { await api.patch(`/api/v1/cars/${carId}`, {video:null}); } catch {} }
                          setVideo("");
                        }}>Remove</button>
                      </div>
                    </div>
                  ) : (
                    <button className="upload-video-btn" onClick={() => vidInputRef.current?.click()} disabled={uploading}>
                ...
                {([
                  ...
                  <div key={f.key} className="field">
                    ...
                    {f.type === "select"
                      ? <select value={form[f.key]||""} onChange={e=>setForm((p:any)=>({...p,[f.key]:e.target.value}))} style={{...fi,cursor:"pointer",textTransform:"capitalize"}}>
                ...
                <div className="field">
                  ...
                  <textarea value={form.description||""} onChange={e=>setForm((p:any)=>({...p,description:e.target.value}))} placeholder="Describe the car, features, condition, history..." rows={4} style={{...fi,resize:"vertical"}} />
                </div>
              </div>
        ...
        <style>{`
          ...
          .page-heading{font-family:var(--font-display);font-size:1.5rem;letter-spacing:0.05em;color:#1A1A1A;line-height:1}
          .page-sub{font-size:0.8rem;color:#737373;margin-top:0.3rem}
          ...
          .search-input{flex:1;min-width:160px;background:#fff;border:1.5px solid #E5E5E5;border-radius:8px;padding:0.625rem 1rem;font-size:0.875rem;font-family:var(--font-body);outline:none;color:#1A1A1A}
          .search-input:focus{border-color:#F47B20}
          .status-select{background:#fff;border:1.5px solid #E5E5E5;border-radius:8px;padding:0.625rem 0.875rem;font-size:0.875rem;font-family:var(--font-body);outline:none;cursor:pointer;color:#1A1A1A}
          ...
          .empty{display:flex;flex-direction:column;align-items:center;gap:0.875rem;padding:3rem 1rem;text-align:center;border:1.5px dashed #E5E5E5;border-radius:12px;background:#FAFAFA}
          .ei{font-size:3rem}
          .empty h3{font-family:var(--font-display);font-size:1.1rem;color:#1A1A1A}
          .empty p{color:#737373;font-size:0.875rem}
          .cars-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(min(240px,100%),1fr));gap:1rem}
          ...
          .car-img{aspect-ratio:4/3;background:#F5F5F5;position:relative;overflow:hidden;cursor:pointer}
          .car-img img{width:100%;height:100%;object-fit:cover;display:block;transition:transform 0.3s}
          ...
          .car-actions{display:grid;grid-template-columns:1fr auto auto auto;gap:0.35rem;padding:0.625rem 0.75rem;border-top:1px solid #F0F0F0}
          .act-btn{background:#F5F5F5;border:1px solid #E5E5E5;border-radius:5px;padding:0.4rem 0.6rem;font-size:0.72rem;cursor:pointer;font-family:var(--font-body);color:#525252;transition:all 0.15s;white-space:nowrap}
          .act-btn:hover{border-color:#F47B20;color:#F47B20;background:#FFF7ED}
          .sold-btn{background:#FFF7ED;border-color:rgba(244,123,32,0.4);color:#C4621A;font-weight:600}
          .sold-btn:hover{background:#F47B20;color:#fff;border-color:#F47B20}
          .report-btn:hover{border-color:#3B8BD4;color:#3B8BD4;background:#EFF6FF}
          .del-btn{color:#DC2626!important;border-color:rgba(220,38,38,0.25)!important}
          .del-btn:hover{background:#FEF2F2!important;border-color:#DC2626!important}
          ...
          .modal{background:#fff;border-radius:16px;width:100%;max-width:600px;margin:auto;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,0.2)}
          .modal-header{display:flex;align-items:center;justify-content:space-between;padding:1.25rem 1.5rem;border-bottom:1.5px solid #E5E5E5;position:sticky;top:0;background:#fff;z-index:10}
          .modal-title{font-family:var(--font-display);font-size:1rem;letter-spacing:0.08em;color:#1A1A1A}
          .draft-note{font-size:0.68rem;color:#16A34A;margin-top:0.15rem}
          ...
          .modal-body{padding:1.25rem 1.5rem;display:flex;flex-direction:column;gap:1rem;max-height:68vh;overflow-y:auto}
          .modal-footer{padding:1rem 1.5rem;border-top:1.5px solid #E5E5E5;display:flex;gap:0.75rem;background:#FAFAFA}
          .modal-footer .btn-primary{flex:2}
          .modal-footer .btn-outline{flex:1}
          .form-error{background:#FEF2F2;border:1px solid #FCA5A5;color:#DC2626;padding:0.65rem 1rem;border-radius:8px;font-size:0.825rem;display:flex;justify-content:space-between;align-items:flex-start;gap:0.5rem}
          .form-error button{background:none;border:none;color:inherit;cursor:pointer}
          .field{display:flex;flex-direction:column;gap:0.4rem}
          .fl{font-size:0.7rem;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;color:#737373}
          .upload-hint{background:#FFF7ED;border:1px solid rgba(244,123,32,0.3);border-radius:6px;padding:0.5rem 0.875rem;font-size:0.72rem;color:#C4621A;line-height:1.5}
          .photo-grid{display:flex;gap:0.5rem;flex-wrap:wrap}
          ...
          .upload-progress{font-size:0.78rem;color:#F47B20}
          .video-preview{display:flex;align-items:center;gap:0.75rem}
          .video-thumb{position:relative;cursor:pointer;width:120px;height:72px;border-radius:6px;overflow:hidden;border:1.5px solid #E5E5E5;flex-shrink:0}
          .video-thumb video{width:100%;height:100%;object-fit:cover}
          .play-icon{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.3);font-size:1.5rem;color:#fff}
          .video-actions{display:flex;flex-direction:column;gap:0.4rem}
          .upload-video-btn{background:#F5F5F5;border:1.5px dashed #D4D4D4;border-radius:8px;padding:0.75rem 1.25rem;font-size:0.825rem;cursor:pointer;color:#737373;font-family:var(--font-body)}
          .upload-video-btn:disabled{cursor:not-allowed}
          @media(max-width:640px){
            .cars-grid{grid-template-columns:1fr 1fr}
            .header-btns{width:100%;justify-content:flex-end}
            .car-actions{grid-template-columns:1fr 1fr 1fr 1fr}
