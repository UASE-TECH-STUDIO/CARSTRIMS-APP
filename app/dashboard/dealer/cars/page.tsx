...
export default function DealerCarsPage() {
  ...
  const ensureCarSaved = async (): Promise<string|null> => {
    ...
    try {
      ...
      setSavedCarId(cid);
      return cid;
    } catch(ex:any) { setErr(ex.response?.data?.detail || "Save car info first before uploading photos"); return null; }
    finally { setSaving(false); }
  };

  const handleImgFiles = async (files: FileList) => {
    if (images.length + files.length > MAX_IMAGES) { setErr(`Maximum ${MAX_IMAGES} photos allowed`); return; }
    setUploading(true);
    try {
      ...
      {markSoldCar && (
        <MarkSoldModal
          ...
          onSold={async (txn) => {
            setMarkSoldCar(null);
            await load();
            alert(`Sale recorded! Transaction ID: ${txn.transactionId || "completed"}`);
          }}
        />
      )}

      {/* Per-car Financial Report */}
      {reportCarId && (
      ...
      <div style={{display:"flex",flexDirection:"column",gap:"1.5rem",fontFamily:"var(--font-body)"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:"1rem",flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",gap:"0.75rem",flexWrap:"wrap"}}>
            <button onClick={()=>setMarkSoldCar({} as Car)}
              style={{background:"#1A1A1A",color:"#fff",border:"none",borderRadius:"8px",padding:"0.75rem 1.25rem",fontFamily:"var(--font-display)",fontSize:"0.825rem",letterSpacing:"0.08em",cursor:"pointer",whiteSpace:"nowrap"}}>
              💳 Record Sale
            </button>
            <button onClick={openAdd}
              style={{background:"#F47B20",color:"#fff",border:"none",borderRadius:"8px",padding:"0.75rem 1.5rem",fontFamily:"var(--font-display)",fontSize:"0.875rem",letterSpacing:"0.08em",cursor:"pointer",whiteSpace:"nowrap"}}>
              + Add Car
            </button>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(min(235px,100%),1fr))",gap:"1rem"}}>
            {cars.map(car=>(
              <div key={car._id} style={{background:"#fff",border:"1.5px solid #E5E5E5",borderRadius:"10px",overflow:"hidden",display:"flex",flexDirection:"column",transition:"border-color 0.2s"}}
                ...
                {/* Image */}
                <div style={{aspectRatio:"4/3",background:"#F5F5F5",position:"relative",overflow:"hidden",cursor:car.images?.[0]?"zoom-in":"default"}}
                  onClick={()=>car.images?.[0]&&setPreview({src:car.images[0],type:"image"})}>
                  {car.images?.[0]
                    ? <img src={car.images[0]} alt="" style={{width:"100%",height:"100%",objectFit:"cover",display:"block"}} />
                ...
                <div style={{padding:"0.875rem",flex:1,display:"flex",flexDirection:"column",gap:"0.25rem"}}>
                  ...
                  <div style={{fontFamily:"var(--font-display)",fontSize:"1.05rem",color:"#F47B20",marginTop:"0.2rem"}}>₦{(car.sellingPrice||0).toLocaleString()}</div>
                  <div style={{fontSize:"0.62rem",color:"#A3A3A3",fontFamily:"monospace"}}>{car.carId}</div>
                </div>

                {/* Action buttons */}
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"0.35rem",padding:"0.75rem",borderTop:"1px solid #F0F0F0"}}>
                  <button onClick={()=>openEdit(car)}
                    style={{background:"#F5F5F5",border:"1px solid #E5E5E5",borderRadius:"6px",padding:"0.45rem 0.25rem",fontSize:"0.72rem",cursor:"pointer",color:"#525252",fontFamily:"var(--font-body)"}}>
                    ✏️ Edit
                  </button>
                  {car.status !== "sold" ? (
                    <button onClick={()=>setMarkSoldCar(car)}
                      style={{background:"#FFF7ED",border:"1px solid rgba(244,123,32,0.35)",borderRadius:"6px",padding:"0.45rem 0.25rem",fontSize:"0.72rem",cursor:"pointer",color:"#C4621A",fontFamily:"var(--font-body)",fontWeight:600}}>
                      💳 Sold
                    </button>
                  ) : (
                    <button onClick={()=>setReportCarId(car.carId)}
                      style={{background:"#F0FDF4",border:"1px solid #86EFAC",borderRadius:"6px",padding:"0.45rem 0.25rem",fontSize:"0.72rem",cursor:"pointer",color:"#16A34A",fontFamily:"var(--font-body)"}}>
                      📊 Report
                    </button>
                  )}
                  <button onClick={()=>handleDelete(car.carId)}
                    style={{background:"#FEF2F2",border:"1px solid rgba(220,38,38,0.2)",borderRadius:"6px",padding:"0.45rem 0.25rem",fontSize:"0.72rem",cursor:"pointer",color:"#DC2626",fontFamily:"var(--font-body)"}}>
                    🗑 Del
                  </button>
                </div>

                {/* Report button for all cars */}
                <div style={{padding:"0 0.75rem 0.75rem"}}>
                  <button onClick={()=>setReportCarId(car.carId)}
                    style={{width:"100%",background:"#F5F5F5",border:"1px solid #E5E5E5",borderRadius:"6px",padding:"0.4rem",fontSize:"0.72rem",cursor:"pointer",color:"#737373",fontFamily:"var(--font-body)",transition:"all 0.15s"}}
                    ...
                    onMouseOut={e=>{e.currentTarget.style.borderColor="#E5E5E5";e.currentTarget.style.color="#737373";e.currentTarget.style.background="#F5F5F5"}}>
                    📊 Financial Report
                  </button>
                </div>
              </div>
          ...
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"flex-start",justifyContent:"center",zIndex:1000,overflowY:"auto",padding:"1rem"}}>
            <div style={{background:"#fff",borderRadius:"16px",width:"100%",maxWidth:"600px",margin:"1rem auto",overflow:"hidden",boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}>
              ...
              <div style={{padding:"1.5rem",display:"flex",flexDirection:"column",gap:"1.25rem",maxHeight:"72vh",overflowY:"auto"}}>
                ...
                {/* PHOTOS */}
                <div>
                  <label style={lbl}>Photos ({images.length}/{MAX_IMAGES}) <span style={{color:"#A3A3A3",fontWeight:400,textTransform:"none",fontSize:"0.68rem"}}>— click any photo to preview</span></label>
                  {modal==="add"&&!savedCarId&&(
                    <div style={{background:"#FFF7ED",border:"1px solid rgba(244,123,32,0.3)",borderRadius:"6px",padding:"0.625rem 0.875rem",fontSize:"0.75rem",color:"#C4621A",marginBottom:"0.5rem",lineHeight:1.5}}>
                      💡 Fill in model and price below first, then click "+" to add photos — we save the car draft automatically.
                    </div>
                  )}
                ...
                <div>
                  ...
                  {video ? (
                    <div style={{display:"flex",alignItems:"center",gap:"0.75rem"}}>
                      <div style={{position:"relative",cursor:"pointer"}} onClick={()=>setPreview({src:video,type:"video"})}>
                        <video src={video} style={{width:"120px",height:"72px",objectFit:"cover",borderRadius:"6px",border:"1.5px solid #E5E5E5"}} />
                        <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.3)",borderRadius:"6px"}}>
                      ...
                      <div style={{display:"flex",flexDirection:"column",gap:"0.4rem"}}>
                        ...
                        <button onClick={async()=>{
                          ...
                          setVideo("");
                        }} style={{background:"#FEF2F2",border:"1px solid rgba(220,38,38,0.3)",color:"#DC2626",borderRadius:"6px",padding:"0.35rem 0.75rem",fontSize:"0.78rem",cursor:"pointer"}}>Remove</button>
                      </div>
                    </div>
                  ) : (
