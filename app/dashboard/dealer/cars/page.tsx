...
const BRANDS = ["Toyota","Honda","Mercedes","BMW","Lexus","Ford","Hyundai","Kia","Chevrolet","Audi","Land Rover","Jeep","Volkswagen","Nissan","Mazda","Peugeot","Kia","Mitsubishi","Subaru","Isuzu","Opel","Renault","Other"];
...
export default function DealerCarsPage() {
  ...
  const fi: React.CSSProperties = { background:"#F5F5F5", border:"1.5px solid #E5E5E5", borderRadius:"8px", padding:"0.8rem 0.875rem", color:"#1A1A1A", fontSize:"0.875rem", fontFamily:"var(--font-body)", outline:"none", width:"100%", boxSizing:"border-box" as const };
  ...
  return (
    <div className="cars-page">
      ...
      <div className="cp-filters">
        <input className="cp-search" placeholder="Search brand, model, ID..." value={search}
        ...
        <div className="cp-status-tabs">
          {["all","available","sold","reserved","out_for_inspection","in_repair"].map((s) => (
            <button key={s} className={`cp-stab ${statusFilter===s?"active":""}`}
              onClick={() => setStatusFilter(s)}>{s==="all"?"All":s.replace(/_/g," ")}</button>
          ))}
        </div>
      </div>
          ...
          {cars.map((c) => (
            <div key={c._id} className="cp-card">
              <div className="cp-img-wrap">
                {c.images?.[0] ? <img src={c.images[0]} alt="" className="cp-img" /> : <div className="cp-img-ph">No Photo</div>}
                <div className="cp-status-badge" style={{background:STATUS_COLORS[c.status]||"#737373"}}>{c.status?.replace(/_/g," ")}</div>
                <div className="cp-img-count">{(c.images||[]).length} / 10 photos</div>
              </div>
              <div className="cp-card-body">
                <div className="cp-car-name">{c.brand} {c.model} {c.year}</div>
                <div className="cp-car-id">{c.carId}</div>
                <div className="cp-car-meta">{c.color} &middot; {c.transmission} &middot; {c.fuelType}</div>
            ...
            <div className="modal-body">
              ...
              <div className="modal-grid2">
                <div>
                  ...
                  <select style={fi} defaultValue={formData.brand} onChange={(e)=>set("brand",e.target.value)}>
                    {BRANDS.map(b=><option key={b}>{b}</option>)}
                  </select>
                </div>
                <div>
                  <label style={lbl}>Model *</label>
                  <input style={fi} placeholder="e.g. Camry" defaultValue={formData.model} onChange={(e)=>set("model",e.target.value)} />
                </div>
                <div>
                  <label style={lbl}>Year *</label>
                  <input type="number" style={fi} defaultValue={formData.year} onChange={(e)=>set("year",e.target.value)} min="1990" max="2030" />
                </div>
                <div>
                  <label style={lbl}>Color *</label>
                  <input style={fi} placeholder="e.g. Black" defaultValue={formData.color} onChange={(e)=>set("color",e.target.value)} />
                </div>
                <div>
                  <label style={lbl}>Condition</label>
                  <select style={fi} defaultValue={formData.condition} onChange={(e)=>set("condition",e.target.value)}>
                    {CONDITIONS.map(c=><option key={c}>{c}</option>)}
                  </select>
                </div>
                <div>
                  <label style={lbl}>Status</label>
                  <select style={fi} defaultValue={formData.status} onChange={(e)=>set("status",e.target.value)}>
              ...
              <div className="modal-grid2">
                <div>
                  ...
                  <input style={fi} placeholder="e.g. Abuja" defaultValue={formData.city} onChange={(e)=>set("city",e.target.value)} />
                </div>
                <div>
                  <label style={lbl}>State</label>
                  <input style={fi} placeholder="e.g. FCT" defaultValue={formData.state} onChange={(e)=>set("state",e.target.value)} />
                </div>
              </div>
              ...
              <div className="modal-grid2">
                <div>
                  ...
                  <select style={fi} defaultValue={formData.fuelType} onChange={(e)=>set("fuelType",e.target.value)}>
                    {FUEL_TYPES.map(f=><option key={f}>{f}</option>)}
                  </select>
                </div>
                <div>
                  <label style={lbl}>Transmission</label>
                  <select style={fi} defaultValue={formData.transmission} onChange={(e)=>set("transmission",e.target.value)}>
                    {TRANS.map(t=><option key={t}>{t}</option>)}
                  </select>
                </div>
                <div>
                  <label style={lbl}>Mileage (km)</label>
                  <input type="number" style={fi} placeholder="0" defaultValue={formData.mileage} onChange={(e)=>set("mileage",e.target.value)} />
                </div>
                <div>
                  <label style={lbl}>Engine Type</label>
                  <input style={fi} placeholder="e.g. V6 3.5L" defaultValue={formData.engineType} onChange={(e)=>set("engineType",e.target.value)} />
                </div>
                <div className="modal-col2">
                  <label style={lbl}>VIN / Chassis Number</label>
                  <input style={fi} placeholder="Optional" defaultValue={formData.vin} onChange={(e)=>set("vin",e.target.value)} />
                </div>
              </div>
              ...
              <div className="modal-grid3">
                <div>
                  ...
                  <input type="number" style={fi} placeholder="0" defaultValue={formData.sellingPrice} onChange={(e)=>set("sellingPrice",e.target.value)} />
                </div>
                <div>
                  <label style={lbl}>Purchase Price</label>
                  <input type="number" style={fi} placeholder="0" defaultValue={formData.purchasePrice} onChange={(e)=>set("purchasePrice",e.target.value)} />
                </div>
                <div>
                  <label style={lbl}>Promo Price</label>
                  <input type="number" style={fi} placeholder="0" defaultValue={formData.promoPrice} onChange={(e)=>set("promoPrice",e.target.value)} />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea style={{...fi, minHeight:"80px", resize:"vertical"}} placeholder="Describe this vehicle..." defaultValue={formData.description} onChange={(e)=>set("description",e.target.value)} />
              </div>

              {/* SECTION: Photos */}
              <div className="modal-section-title">PHOTOS ({images.length}/10)</div>
              <div className="media-grid">
                ...
                {images.length < 10 && (
                  <button className="media-add-btn" onClick={() => imgInputRef.current?.click()} disabled={uploading}>
                    {uploading && uploadProgress.includes("photo") ? "Uploading..." : "+ Add Photo"}
                  </button>
                )}
              </div>
              <input ref={imgInputRef} type="file" accept="image/jpeg,image/png,image/webp" multiple style={{display:"none"}}
                onChange={(e) => { if(e.target.files?.length) handleImgFiles(e.target.files); }} />
              <p className="media-hint">First photo is the main display image. Max 10 photos. JPG, PNG, WebP.</p>

              {/* SECTION: Video */}
              <div className="modal-section-title">VIDEO (optional)</div>
              {video ? (
                ...
                <button className="media-add-btn media-video-btn" onClick={() => vidInputRef.current?.click()} disabled={uploading}>
                  {uploading && uploadProgress.includes("video") ? "Uploading video..." : "+ Upload Video (max 50MB)"}
                </button>
              )}
              <input ref={vidInputRef} type="file" accept="video/mp4,video/mov,video/avi,video/webm" style={{display:"none"}}
      ...
      <style>{`
        ...
        .cp-add-btn { background:#F47B20; color:#fff; border:none; border-radius:8px; padding:0.7rem 1.25rem; font-family:var(--font-display); font-size:0.875rem; letter-spacing:0.08em; cursor:pointer; white-space:nowrap; flex-shrink:0; }
        .cp-add-btn:hover { background:#FF9340; }

        .cp-filters { display:flex; flex-direction:column; gap:0.75rem; }
        .cp-search { background:#fff; border:1.5px solid #E5E5E5; border-radius:8px; padding:0.7rem 1rem; color:#1A1A1A; font-size:0.875rem; font-family:var(--font-body); outline:none; width:100%; box-sizing:border-box; }
        ...
        .cp-img-ph { font-size:0.8rem; color:#A3A3A3; }
        ...
        .cp-car-id { font-family:var(--font-mono); font-size:0.68rem; color:#A3A3A3; }
        .cp-car-meta { font-size:0.72rem; color:#737373; text-transform:capitalize; }
        .cp-car-price { font-family:var(--font-display); font-size:1.1rem; color:#F47B20; margin-top:0.2rem; }
        .cp-card-actions { display:flex; gap:0.5rem; margin-top:0.5rem; padding-top:0.625rem; border-top:1px solid #F5F5F5; }
        .cp-edit-btn { flex:1; background:#FFF7ED; border:1.5px solid rgba(244,123,32,0.3); color:#F47B20; border-radius:6px; padding:0.5rem; font-size:0.78rem; cursor:pointer; font-family:var(--font-body); }
        .cp-edit-btn:hover { background:#F47B20; color:#fff; }
        .cp-del-btn { background:#FEF2F2; border:1.5px solid rgba(220,38,38,0.2); color:#DC2626; border-radius:6px; padding:0.5rem 0.75rem; font-size:0.78rem; cursor:pointer; font-family:var(--font-body); }
        .cp-del-btn:hover { background:#DC2626; color:#fff; }

        /* MODAL */
        .modal-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.5); display:flex; align-items:flex-start; justify-content:center; z-index:1000; padding:1rem; overflow-y:auto; }
        ...
        .modal-x { background:#F5F5F5; border:none; width:32px; height:32px; border-radius:50%; cursor:pointer; font-size:0.875rem; font-weight:700; color:#737373; display:flex; align-items:center; justify-content:center; }
        .modal-x:hover { background:#E5E5E5; }
        .modal-err { margin:0 1.5rem; padding:0.75rem 1rem; background:#FEF2F2; border:1px solid #FCA5A5; color:#DC2626; border-radius:8px; font-size:0.825rem; flex-shrink:0; }
        .modal-progress { margin:0 1.5rem; padding:0.75rem 1rem; background:#FFF7ED; border:1px solid rgba(244,123,32,0.3); color:#F47B20; border-radius:8px; font-size:0.825rem; flex-shrink:0; }
        .modal-body { padding:1.5rem; overflow-y:auto; flex:1; display:flex; flex-direction:column; gap:1.25rem; }
        .modal-section-title { font-family:var(--font-display); font-size:0.72rem; letter-spacing:0.15em; color:#A3A3A3; border-bottom:1px solid #F5F5F5; padding-bottom:0.5rem; }
        ...
        .modal-footer { display:flex; gap:0.75rem; padding:1.25rem 1.5rem; border-top:1.5px solid #F5F5F5; flex-shrink:0; }
        .modal-cancel { background:#F5F5F5; border:1.5px solid #E5E5E5; color:#525252; border-radius:8px; padding:0.875rem 1.5rem; font-family:var(--font-body); font-size:0.875rem; cursor:pointer; }
        .modal-save { flex:1; background:#F47B20; color:#fff; border:none; border-radius:8px; padding:0.875rem; font-family:var(--font-display); font-size:0.95rem; letter-spacing:0.08em; cursor:pointer; }
        ...
        .media-main-badge { position:absolute; bottom:3px; left:3px; background:#F47B20; color:#fff; font-size:0.55rem; font-weight:700; padding:0.1rem 0.4rem; border-radius:3px; letter-spacing:0.05em; }
        .media-add-btn { aspect-ratio:4/3; background:#F5F5F5; border:1.5px dashed #D4D4D4; border-radius:8px; color:#737373; font-size:0.78rem; cursor:pointer; display:flex; align-items:center; justify-content:center; font-family:var(--font-body); transition:all 0.2s; }
        .media-add-btn:hover { border-color:#F47B20; color:#F47B20; background:#FFF7ED; }
        .media-add-btn:disabled { opacity:0.5; cursor:not-allowed; }
        .media-video-btn { aspect-ratio:unset; padding:1rem; width:100%; }
        .media-hint { font-size:0.72rem; color:#A3A3A3; }
        .video-preview { display:flex; flex-direction:column; gap:0.75rem; }
        .video-player { width:100%; max-height:200px; border-radius:8px; background:#000; }
        .media-remove-video { background:none; border:1.5px solid #FCA5A5; color:#DC2626; border-radius:6px; padding:0.5rem 1rem; font-size:0.8rem; cursor:pointer; font-family:var(--font-body); align-self:flex-start; }
