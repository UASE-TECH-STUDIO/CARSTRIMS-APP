...
const CLOUD = "https://api.cloudinary.com/v1_1/dkvj0wjta/auto/upload";
const PRESET = "carstrims_cars";
...
export default function DealerSetupPage() {
  ...
  const bizIdRef = useRef<HTMLInputElement>(null);
  ...
  const upload = async (file: File, setter: (u: string) => void, label: string) => {
    setUploading(label);
    try { setter(await uploadToCloud(file)); }
    catch { setError(`${label} upload failed. Please try again.`); }
    finally { setUploading(""); }
  };

  const togglePerm = (p: string) => setStaff(f => ({ ...f, permissions: f.permissions.includes(p) ? f.permissions.filter(x => x !== p) : [...f.permissions, p] }));
  ...
  const handleStep1 = async (e: React.FormEvent) => {
    ...
    if (!logoUrl) { setError("Please upload your company/business logo"); return; }
    setLoading(true); setError("");
    try {
      await api.post("/api/v1/dealers/setup", {
        ...company,
        logo: logoUrl,
  ...
  const handleStep2 = async (skip?: boolean) => {
    ...
    if (!idUrl) { setError("Please upload a valid government-issued ID"); return; }
    setLoading(true); setError("");
    try {
      await api.patch("/api/v1/dealers/me", {
        idCardUrl: idUrl,
        cacUrl: cacUrl || undefined,
        businessIdUrl: businessIdUrl || undefined,
        isRegisteredBusiness: isRegistered,
      });
      setStep(3);
    } catch (err: any) {
      setError(err.response?.data?.detail || "Failed to save documents.");
    } finally { setLoading(false); }
  };
  ...
  const handleStep5 = async (skip?: boolean) => {
    if (!skip && cctv.cameraName && cctv.streamUrl) {
      setLoading(true);
      try { await api.post("/api/v1/cctv/", cctv); }
      catch { /* non-critical */ }
      finally { setLoading(false); }
    }
  ...
  const UploadBox = ({ label, url, onFile, inputRef, accept="image/*", note="" }: { label:string, url:string, onFile:(f:File)=>void, inputRef:React.RefObject<HTMLInputElement>, accept?:string, note?:string }) => (
    <div style={{border:`1.5px dashed ${url?"#16A34A":"#D4D4D4"}`, borderRadius:"10px", padding:"1rem", background:url?"#F0FDF4":"#FAFAFA", display:"flex", flexDirection:"column", alignItems:"center", gap:"0.5rem", cursor:"pointer", position:"relative"}} onClick={() => inputRef.current?.click()}>
      {url ? (
        <>
          <div style={{fontSize:"1.5rem"}}>✅</div>
          <div style={{fontSize:"0.75rem", color:"#16A34A", fontWeight:600}}>{label} uploaded</div>
          {accept.includes("image") && <img src={url} alt="" style={{width:"60px", height:"60px", objectFit:"cover", borderRadius:"6px", border:"1.5px solid #86EFAC"}} />}
          <button onClick={e=>{e.stopPropagation(); inputRef.current?.click();}} style={{background:"none", border:"1px solid #86EFAC", color:"#16A34A", borderRadius:"5px", padding:"0.2rem 0.6rem", fontSize:"0.7rem", cursor:"pointer"}}>Change</button>
        </>
      ) : (
        <>
          <div style={{fontSize:"1.75rem", opacity:0.4}}>{accept.includes("image")?"🖼️":"📄"}</div>
          <div style={{fontSize:"0.8rem", color:"#737373", fontWeight:600, textAlign:"center"}}>{label}</div>
          {note && <div style={{fontSize:"0.7rem", color:"#A3A3A3", textAlign:"center"}}>{note}</div>}
          <div style={{fontSize:"0.72rem", color:"#F47B20", fontWeight:600}}>{uploading===label?"Uploading...":"Click to upload"}</div>
        </>
      )}
            ...
            {STEPS.map((s, i) => (
              <div key={s.num} style={{display:"flex", alignItems:"center", flex:i<STEPS.length-1?1:"auto"}}>
                <div style={{display:"flex", alignItems:"center", gap:"0.4rem", flexShrink:0}}>
                  <div style={{width:"28px", height:"28px", borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"0.75rem", fontWeight:700, background:step>s.num?"#16A34A":step===s.num?"#F47B20":"#E5E5E5", color:step>=s.num?"#fff":"#737373", transition:"all 0.25s", flexShrink:0}}>
                    {step>s.num?"✓":s.num}
                  </div>
                  <span style={{fontSize:"0.68rem", color:step===s.num?"#F47B20":step>s.num?"#16A34A":"#A3A3A3", fontWeight:step===s.num?700:400, whiteSpace:"nowrap"}}>
                    {s.label}
                  </span>
                </div>
      ...
      <div style={{maxWidth:"780px", margin:"0 auto", padding:"2rem 1.5rem 5rem"}}>
        ...
        {step===1 && (
          <div style={{background:"#fff", borderRadius:"16px", padding:"2rem", boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
            ...
            <p style={{fontSize:"0.85rem", color:"#737373", marginBottom:"1.5rem", lineHeight:1.6}}>Tell us about your dealership and upload your photos for verification.</p>
            ...
            <form onSubmit={handleStep1} style={{display:"flex", flexDirection:"column", gap:"1.25rem"}}>
              ...
              <div>
                ...
                <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1rem"}}>
                  <UploadBox label="Business Logo" url={logoUrl} inputRef={logoRef} onFile={f=>upload(f, setLogoUrl, "Business Logo")} note="Your company/dealership logo" />
                  <UploadBox label="Your Passport Photo" url={passportUrl} inputRef={passportRef} onFile={f=>upload(f, setPassportUrl, "Your Passport Photo")} note="Clear front-facing photo" />
                </div>
              </div>
              ...
              <div><label style={lbl}>Business Description</label><textarea style={{...fi, minHeight:"70px", resize:"vertical" as const}} placeholder="What does your dealership specialise in?" value={company.description} onChange={e=>setCompany({...company,description:e.target.value})} /></div>

              <button type="submit" disabled={loading||!!uploading} style={{background:"#F47B20", color:"#fff", border:"none", borderRadius:"8px", padding:"1rem", fontFamily:"var(--font-display)", fontSize:"0.95rem", letterSpacing:"0.1em", cursor:"pointer", opacity:loading||uploading?0.6:1}}>
                {loading?"Saving...":uploading?`Uploading ${uploading}...`:"CONTINUE →"}
              </button>
            </form>
          </div>
        )}
        ...
        {step===2 && (
          <div style={{background:"#fff", borderRadius:"16px", padding:"2rem", boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
            ...
            <p style={{fontSize:"0.85rem", color:"#737373", marginBottom:"1.5rem", lineHeight:1.6}}>Upload your ID and business documents to speed up approval. You can take a photo or upload from your gallery.</p>

            <div style={{display:"flex", flexDirection:"column", gap:"1.5rem"}}>
              {/* Government ID — REQUIRED */}
              <div>
                <label style={lbl}>Government-Issued ID * <span style={{color:"#F47B20"}}>(Required)</span></label>
                <p style={{fontSize:"0.75rem", color:"#737373", marginBottom:"0.5rem"}}>National ID, Voter's Card, Driver's Licence, or International Passport</p>
                <UploadBox label="Upload ID Card" url={idUrl} inputRef={idRef} onFile={f=>upload(f, setIdUrl, "ID Card")} note="Take a photo or upload from gallery" />
              </div>

              {/* Business registration */}
              <div>
                <label style={lbl}>Is your business registered?</label>
                <div style={{display:"flex", gap:"0.75rem", marginBottom:"0.75rem"}}>
                  {[{v:true,l:"Yes, registered"},{v:false,l:"No, not registered"}].map(o=>(
                    <button key={String(o.v)} onClick={()=>setIsRegistered(o.v)} type="button"
                      style={{flex:1, padding:"0.75rem", borderRadius:"8px", border:`1.5px solid ${isRegistered===o.v?"#F47B20":"#E5E5E5"}`, background:isRegistered===o.v?"#FFF7ED":"#F5F5F5", color:isRegistered===o.v?"#C4621A":"#525252", fontSize:"0.85rem", cursor:"pointer", fontFamily:"var(--font-body)", fontWeight:isRegistered===o.v?600:400}}>
                      {o.l}
                    </button>
                  ))}
                </div>
                {isRegistered && (
                  <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1rem"}}>
                    <div>
                      <label style={{...lbl, marginBottom:"0.5rem"}}>CAC Certificate <span style={{color:"#A3A3A3", fontWeight:400}}>(optional)</span></label>
                      <UploadBox label="Upload CAC Doc" url={cacUrl} inputRef={cacRef} onFile={f=>upload(f, setCacUrl, "CAC Certificate")} accept="image/*,.pdf" note="CAC registration certificate" />
                    </div>
                    <div>
                      <label style={{...lbl, marginBottom:"0.5rem"}}>Business ID Card <span style={{color:"#A3A3A3", fontWeight:400}}>(optional)</span></label>
                      <UploadBox label="Upload Business ID" url={businessIdUrl} inputRef={bizIdRef} onFile={f=>upload(f, setBusinessIdUrl, "Business ID")} note="Company staff ID card" />
                    </div>
                  </div>
                )}
              </div>
              ...
              <div style={{background:"#FFF7ED", border:"1px solid rgba(244,123,32,0.3)", borderRadius:"8px", padding:"0.875rem", fontSize:"0.8rem", color:"#C4621A", lineHeight:1.6}}>
                📋 Your documents are encrypted and only viewed by the CARSTRIMS admin team for verification purposes.
              </div>
              ...
              <div style={{display:"flex", gap:"0.75rem"}}>
                ...
                <button onClick={()=>handleStep2()} disabled={loading||!!uploading} style={{flex:1, background:"#F47B20", color:"#fff", border:"none", borderRadius:"8px", padding:"0.875rem", fontFamily:"var(--font-display)", fontSize:"0.95rem", letterSpacing:"0.08em", cursor:"pointer", opacity:loading||uploading?0.6:1}}>
                  {loading?"Saving...":uploading?`Uploading ${uploading}...`:"CONTINUE →"}
                </button>
        ...
        {step===3 && (
          <div style={{background:"#fff", borderRadius:"16px", padding:"2rem", boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
            ...
            <p style={{fontSize:"0.85rem", color:"#737373", marginBottom:"1.5rem", lineHeight:1.6}}>List your first vehicle. You can add photos later from your Cars dashboard.</p>
            ...
            <form onSubmit={handleStep3} style={{display:"flex", flexDirection:"column", gap:"1.25rem"}}>
              <div style={row}>
                <div><label style={lbl}>Brand *</label>
                  <select style={{...fi, cursor:"pointer"}} value={car.brand} onChange={e=>setCar({...car,brand:e.target.value})}>
                    {["Toyota","Honda","Mercedes","BMW","Lexus","Ford","Hyundai","Kia","Chevrolet","Nissan","Mazda","Audi","Land Rover","Jeep","Volkswagen","Peugeot","Mitsubishi","Subaru","Other"].map(b=><option key={b} value={b}>{b}</option>)}
                  </select>
                </div>
              ...
              <div style={row}>
                <div><label style={lbl}>Year *</label><input type="number" style={fi} value={car.year} onChange={e=>setCar({...car,year:Number(e.target.value)})} required /></div>
                <div><label style={lbl}>Color *</label><input style={fi} placeholder="e.g. Black, White..." value={car.color} onChange={e=>setCar({...car,color:e.target.value})} required /></div>
              </div>
              <div style={row}>
        ...
        {step===4 && (
          <div style={{background:"#fff", borderRadius:"16px", padding:"2rem", boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
            <h2 style={{fontFamily:"var(--font-display)", fontSize:"1.4rem", letterSpacing:"0.04em", color:"#1A1A1A", marginBottom:"0.25rem"}}>Add Your First Staff Member</h2>
            <p style={{fontSize:"0.85rem", color:"#737373", marginBottom:"1.5rem", lineHeight:1.6}}>Create a staff account and set their permissions. They can sign in with these details.</p>
            ...
            <form onSubmit={handleStep4} style={{display:"flex", flexDirection:"column", gap:"1.25rem"}}>
              ...
              <div style={row}>
                <div><label style={lbl}>Position / Role</label><input style={fi} placeholder="Sales Manager" value={staff.position} onChange={e=>setStaff({...staff,position:e.target.value})} /></div>
                <div><label style={lbl}>Temporary Password</label><input style={fi} value={staff.password} onChange={e=>setStaff({...staff,password:e.target.value})} /></div>
              </div>
                ...
                <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0.5rem", marginTop:"0.25rem"}}>
                  {PERMISSIONS.map(p=>(
                    <label key={p} style={{display:"flex", alignItems:"center", gap:"0.5rem", padding:"0.5rem 0.75rem", border:`1.5px solid ${staff.permissions.includes(p)?"#F47B20":"#E5E5E5"}`, borderRadius:"6px", cursor:"pointer", fontSize:"0.78rem", color:staff.permissions.includes(p)?"#C4621A":"#737373", background:staff.permissions.includes(p)?"#FFF7ED":"#F5F5F5", transition:"all 0.15s"}}>
                      <input type="checkbox" checked={staff.permissions.includes(p)} onChange={()=>togglePerm(p)} style={{accentColor:"#F47B20"}} />
        ...
        {step===5 && (
          <div style={{background:"#fff", borderRadius:"16px", padding:"2rem", boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
            <h2 style={{fontFamily:"var(--font-display)", fontSize:"1.4rem", letterSpacing:"0.04em", color:"#1A1A1A", marginBottom:"0.25rem"}}>CCTV Setup <span style={{fontSize:"0.9rem", color:"#A3A3A3", fontWeight:400}}>(Optional)</span></h2>
            <p style={{fontSize:"0.85rem", color:"#737373", marginBottom:"1.5rem", lineHeight:1.6}}>Connect a security camera to monitor your dealership. You can add more cameras from Settings later.</p>
            ...
            <div style={{display:"flex", flexDirection:"column", gap:"1.25rem"}}>
              ...
              <div><label style={lbl}>Stream URL</label><input style={fi} placeholder="rtsp://192.168.1.x:554/stream" value={cctv.streamUrl} onChange={e=>setCctv({...cctv,streamUrl:e.target.value})} /></div>
              ...
              <div style={{background:"#F0FDF4", border:"1px solid #86EFAC", borderRadius:"10px", padding:"1.25rem"}}>
                <div style={{fontFamily:"var(--font-display)", fontSize:"0.8rem", letterSpacing:"0.1em", color:"#15803D", marginBottom:"0.75rem"}}>SETUP SUMMARY</div>
                {[
                  {label:"Company profile", done:true},
                  {label:"Verification documents", done:!!idUrl},
                  {label:"First car listed", done:!!car.model},
                  {label:"First staff created", done:!!staff.email},
                ].map(item=>(
                  <div key={item.label} style={{display:"flex", alignItems:"center", gap:"0.5rem", marginBottom:"0.4rem", fontSize:"0.825rem", color:item.done?"#15803D":"#737373"}}>
                    <span>{item.done?"✅":"⬜"}</span><span>{item.label}</span>
                  </div>
                ))}
                <div style={{marginTop:"0.75rem", paddingTop:"0.75rem", borderTop:"1px solid rgba(22,163,74,0.2)", fontSize:"0.8rem", color:"#15803D", lineHeight:1.6}}>
                  After submitting, a CARSTRIMS admin will review your profile. You will have full dashboard access immediately — your listings will appear on the public feed once approved.
                </div>
              </div>
              ...
              <div style={{display:"flex", gap:"0.75rem"}}>
                ...
                <button onClick={()=>handleStep5()} disabled={loading} style={{flex:1, background:"#F47B20", color:"#fff", border:"none", borderRadius:"8px", padding:"0.875rem", fontFamily:"var(--font-display)", fontSize:"0.95rem", letterSpacing:"0.08em", cursor:"pointer", opacity:loading?0.6:1}}>
                  {loading?"Saving...":"SAVE CCTV & GO TO DASHBOARD →"}
                </button>
                <button onClick={()=>handleStep5(true)} style={{background:"#F5F5F5", border:"1.5px solid #E5E5E5", color:"#737373", borderRadius:"8px", padding:"0.875rem 1rem", fontSize:"0.8rem", cursor:"pointer", fontFamily:"var(--font-body)", whiteSpace:"nowrap"}}>Skip</button>
              </div>
            </div>
          </div>
        )}
