...
const Page = dynamic(
  () => import("@/app/dashboard/dealer/partners/page"),
  { ssr: false, loading: () => <div style={{padding:"2rem",color:"#737373"}}>Loading Partners...</div> }
);
...
export default function StaffPartnersPage() {
  ...
  useEffect(() => {
    api.get("/api/v1/staff/me").then(r => {
      ...
      setAllowed(p.includes("view_partners") || p.includes("manage_partners"));
  ...
  if (!allowed) return (
    <div style={{padding:"3rem",textAlign:"center",background:"#fff",border:"1.5px solid #E5E5E5",borderRadius:"12px"}}>
      ...
      <p style={{color:"#737373",marginTop:"0.5rem",fontSize:"0.875rem"}}>You need permission to access Partners. Contact your dealer admin.</p>
    </div>
  );
  return <Page />;
}
