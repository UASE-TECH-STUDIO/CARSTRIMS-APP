...
export default function StaffInventoryAddPage() {
  const [allowed, setAllowed] = useState<boolean|null>(null);

  useEffect(() => {
    api.get("/api/v1/staff/me").then(r => {
      const p: string[] = r.data.permissions || [];
      setAllowed(p.includes("add_cars"));
  ...
  if (!allowed) return (
    <div style={{padding:"3rem",textAlign:"center",background:"#fff",border:"1.5px solid #E5E5E5",borderRadius:"12px"}}>
      ...
      <p style={{color:"#737373",marginTop:"0.5rem",fontSize:"0.875rem"}}>You need add_cars permission to add vehicles.</p>
    </div>
  );
  return <CarsPage />;
}
