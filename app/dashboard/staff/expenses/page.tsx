...
const Page = dynamic(
  () => import("@/app/dashboard/dealer/expenses/page"),
  { ssr: false, loading: () => <div style={{padding:"2rem",color:"#737373"}}>Loading Expenses...</div> }
);
...
export default function StaffExpensesPage() {
  ...
  useEffect(() => {
    api.get("/api/v1/staff/me").then(r => {
      ...
      setAllowed(p.includes("view_expenses") || p.includes("manage_expenses"));
  ...
  if (!allowed) return (
    <div style={{padding:"3rem",textAlign:"center",background:"#fff",border:"1.5px solid #E5E5E5",borderRadius:"12px"}}>
      ...
      <p style={{color:"#737373",marginTop:"0.5rem",fontSize:"0.875rem"}}>You need permission to access Expenses. Contact your dealer admin.</p>
    </div>
  );
  return <Page />;
}
