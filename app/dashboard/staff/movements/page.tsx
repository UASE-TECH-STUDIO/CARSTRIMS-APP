...
const Page = dynamic(
  () => import("@/app/dashboard/dealer/movements/page"),
  { ssr: false, loading: () => <div style={{padding:"2rem",color:"#737373"}}>Loading Vehicle Movements...</div> }
);
...
export default function StaffMovementsPage() {
  ...
  useEffect(() => {
    api.get("/api/v1/staff/me").then(r => {
      ...
      setAllowed(p.includes("view_movements") || p.includes("manage_movements"));
  ...
  if (!allowed) return (
    <div style={{padding:"3rem",textAlign:"center",background:"#fff",border:"1.5px solid #E5E5E5",borderRadius:"12px"}}>
      ...
      <p style={{color:"#737373",marginTop:"0.5rem",fontSize:"0.875rem"}}>You need permission to access Vehicle Movements. Contact your dealer admin.</p>
    </div>
  );
  return <Page />;
}
