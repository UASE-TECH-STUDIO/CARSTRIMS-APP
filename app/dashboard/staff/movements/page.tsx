...
export default function StaffPage() {
  ...
  useEffect(() => {
    api.get("/api/v1/staff/me").then((r) => {
      setAllowed(r.data.permissions?.includes("view_movements") || false);
  ...
  return (
    <div style={{display:"flex", flexDirection:"column", gap:"1.5rem"}}>
      <h2 style={{fontFamily:"var(--font-display)", fontSize:"1.6rem", color:"#1A1A1A", letterSpacing:"0.04em"}}>Vehicle Movements</h2>
      <div style={{background:"#fff", border:"1.5px solid #E5E5E5", borderRadius:"12px", padding:"2rem", textAlign:"center"}}>
        ...
        <Link href="/dashboard/dealer/movements" style={{background:"#F47B20", color:"#fff", borderRadius:"8px", padding:"0.75rem 1.5rem", fontFamily:"var(--font-display)", fontSize:"0.875rem", letterSpacing:"0.08em", textDecoration:"none"}}>
          Go to Vehicle Movements
        </Link>
      </div>
    </div>
  );
