...
export default function StaffSettingsPage() {
  ...
  const [form, setForm] = useState({ fullName:"", phone:"", whatsapp:"", address:"" });
  ...
  const loadStaff = async () => {
    try {
      ...
      setStaff(res.data);
      setForm({ fullName:res.data.fullName||"", phone:res.data.phone||"", whatsapp:res.data.whatsapp||"", address:res.data.address||"" });
    } catch { }
  };
  ...
  const saveProfile = async (e: React.FormEvent) => {
    ...
    try {
      await api.patch("/api/v1/staff/me/profile", form);
      setMsg("Profile updated!"); loadStaff();
  ...
  const uploadPhoto = async (file: File) => {
    ...
    try {
      ...
      setMsg("Profile photo updated!"); loadStaff();
  ...
  return (
    <div className="settings">
      ...
      <p className="page-sub">Update your profile, photo and password</p>
      ...
      <div className="settings-grid">
        ...
        <div className="settings-card">
          ...
          <div className="photo-section">
            <div className="photo-wrap" onClick={()=>fileRef.current?.click()}>
              {staff?.profilePicture
                ? <img src={staff.profilePicture} alt="" className="photo-img" />
                : <div className="photo-placeholder">{user?.fullName?.charAt(0).toUpperCase()||"S"}</div>
              }
          ...
          <div className="photo-name">{staff?.fullName||user?.fullName||"—"}</div>
          <div className="photo-pos">{staff?.position||"Staff Member"}</div>
          <div className="photo-id">{staff?.staffId||"—"}</div>

          {/* Account Info */}
          <div className="info-list">
            <div className="info-row"><span className="il">Email</span><span className="iv">{user?.email}</span></div>
            <div className="info-row"><span className="il">Role</span><span className="iv">Dealer Staff</span></div>
            <div className="info-row"><span className="il">Status</span><span className="iv" style={{color:staff?.status==="active"?"#1D9E75":"#DC2626"}}>{staff?.status||"active"}</span></div>
          </div>
        </div>
        ...
        <div className="settings-card">
          ...
          <form onSubmit={changePw} className="form">
            ...
            <div className="field"><label className="fl">Confirm Password</label><input type="password" className="fi" value={pw.confirmPassword} onChange={(e)=>setPw({...pw,confirmPassword:e.target.value})} required/></div>
        ...
        {staff?.permissions?.length > 0 && (
          <div className="settings-card">
            <div className="card-title">MY PERMISSIONS</div>
            ...
            <p className="perms-note">Permissions are set by your dealer admin. Contact them to update.</p>
          </div>
        )}
      </div>
      ...
      <style>{`
        ...
        .success-banner{background:#F0FDF4;border:1px solid #1D9E75;color:#166534;padding:0.875rem 1.25rem;border-radius:8px;font-size:0.875rem;display:flex;align-items:center;justify-content:space-between}
        ...
        .photo-wrap{position:relative;width:80px;height:80px;border-radius:50%;overflow:hidden;cursor:pointer;border:3px solid #1D9E75}
        .photo-img{width:100%;height:100%;object-fit:cover;display:block}
        .photo-placeholder{width:80px;height:80px;border-radius:50%;background:#1D9E75;color:#fff;font-family:var(--font-display);font-size:2rem;display:flex;align-items:center;justify-content:center}
        .photo-overlay{position:absolute;inset:0;background:rgba(0,0,0,0.5);color:#fff;font-size:0.65rem;display:flex;align-items:center;justify-content:center;opacity:0;transition:opacity 0.2s;text-align:center}
        .photo-wrap:hover .photo-overlay{opacity:1}
        .photo-name{font-family:var(--font-display);font-size:1.1rem;color:#1A1A1A;text-align:center}
        .photo-pos{font-size:0.78rem;color:#1D9E75;text-align:center}
        .photo-id{font-family:var(--font-mono);font-size:0.65rem;color:#AAA;text-align:center}
        ...
        .save-btn{background:#1D9E75;color:#fff;border:none;border-radius:6px;padding:0.75rem 1.5rem;font-family:var(--font-display);font-size:0.9rem;letter-spacing:0.1em;cursor:pointer;align-self:flex-start;transition:background 0.2s}
        .save-btn:hover{background:#16A34A}
        .save-btn:disabled{opacity:0.6;cursor:not-allowed}
        .perms-display{display:grid;grid-template-columns:1fr 1fr;gap:0.4rem}
        .perm-chip{display:flex;align-items:center;gap:0.5rem;padding:0.45rem 0.65rem;background:#F0FDF4;border:1px solid rgba(29,158,117,0.25);border-radius:5px;font-size:0.78rem;color:#1D9E75;text-transform:capitalize}
        .perm-icon{font-size:0.75rem}
        .perms-note{font-size:0.72rem;color:#AAA;font-style:italic}
        .dev-footer{text-align:center;font-size:0.7rem;color:#CCC;padding:1rem}
        .dev-footer strong{color:#1D9E75}
        @media(max-width:640px){.form-row{grid-template-columns:1fr}.perms-display{grid-template-columns:1fr}}
      `}</style>
    </div>
  );
}
