...
const Page = dynamic(
  () => import("@/app/dashboard/dealer/staff/page"),
  { ssr: false, loading: () => <div style={{padding:"2rem",color:"#737373"}}>Loading Staff Management...</div> }
);
...
export default function StaffStaffPage() {
  ...
  useEffect(() => {
    api.get("/api/v1/staff/me").then(r => {
      ...
      setAllowed(p.includes("view_staff") || p.includes("create_staff"));
  ...
  if (!allowed) return (
    <div style={{padding:"3rem",textAlign:"center",background:"#fff",border:"1.5px solid #E5E5E5",borderRadius:"12px"}}>
      ...
      <p style={{color:"#737373",marginTop:"0.5rem",fontSize:"0.875rem"}}>You need permission to access Staff Management. Contact your dealer admin.</p>
    </div>
  );
  return <Page />;
}
