  ...
  const toggle = async () => {
    ...
    try {
      if (following) {
        ...
        setFollowing(false);
        setCount(c => { const n = Math.max(0, c - 1); onChange(n); return n; });
      } else {
        await api.post(`/api/v1/follows/${dealerId}`);
        setFollowing(true);
        setCount(c => { const n = c + 1; onChange(n); return n; });
      }
    } catch {}
...
export default function DealerProfilePage() {
  ...
  const handleMessage = async () => {
    ...
    try {
      ...
      const r = await api.post("/api/v1/messages/start", {
        receiverId,
        message: `Hi, I found your dealership on CARSTRIMS and would like to know more about your available vehicles.`,
      });
      ...
      alert(e.response?.data?.detail || "Could not start chat. Try WhatsApp or Email.");
  ...
  if (loading) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:"#F5F5F5",flexDirection:"column",gap:"1rem"}}>
      <img src="/logo.png" alt="CARSTRIMS" style={{height:"32px",objectFit:"contain"}}/>
  ...
  const socials = [
    dealer.instagram && { label:"Instagram", url: dealer.instagram.startsWith("http") ? dealer.instagram : `https://instagram.com/${dealer.instagram.replace("@","")}`, icon:"📸" },
    dealer.twitter   && { label:"X / Twitter", url: dealer.twitter.startsWith("http")   ? dealer.twitter   : `https://twitter.com/${dealer.twitter.replace("@","")}`, icon:"𝕏" },
    dealer.facebook  && { label:"Facebook",  url: dealer.facebook.startsWith("http")  ? dealer.facebook  : `https://facebook.com/${dealer.facebook}`, icon:"👤" },
    dealer.tiktok    && { label:"TikTok",    url: dealer.tiktok.startsWith("http")    ? dealer.tiktok    : `https://tiktok.com/@${dealer.tiktok.replace("@","")}`, icon:"🎵" },
    dealer.youtube   && { label:"YouTube",   url: dealer.youtube.startsWith("http")   ? dealer.youtube   : `https://youtube.com/${dealer.youtube}`, icon:"▶" },
    dealer.website   && { label:"Website",   url: dealer.website, icon:"🌐" },
  ...
  return (
    <div style={{minHeight:"100vh",background:"#F5F5F5",fontFamily:"var(--font-body)"}}>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:"1rem"}}>
          <div style={{background:"#fff",borderRadius:"14px",width:"100%",maxWidth:"400px",maxHeight:"80vh",overflow:"hidden",display:"flex",flexDirection:"column",boxShadow:"0 16px 48px rgba(0,0,0,0.2)"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"1rem 1.25rem",background:"#F47B20",color:"#fff"}}>
              ...
              <button onClick={()=>setShowFollowers(false)} style={{background:"none",border:"none",color:"#fff",cursor:"pointer",fontSize:"1.1rem"}}>✕</button>
            </div>
            <div style={{flex:1,overflowY:"auto",padding:"0.75rem",display:"flex",flexDirection:"column",gap:"0.25rem"}}>
              {followers.length === 0 ? (
                ...
                <Link key={f.userId || i} href={`/users/${f.userId}`}
                  ...
                  <div style={{width:"38px",height:"38px",borderRadius:"50%",background:"#F5F5F5",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"var(--font-display)",fontSize:"1rem",color:"#F47B20",flexShrink:0,overflow:"hidden"}}>
                  ...
                  <span style={{fontSize:"0.72rem",color:"#F47B20"}}>→</span>
                </Link>
              ))}
            </div>
          </div>
      ...
      <header style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0.875rem 1.5rem",background:"#fff",borderBottom:"1.5px solid #E5E5E5",position:"sticky",top:0,zIndex:40}}>
        ...
        <Link href="/feed"><img src="/logo.png" alt="CARSTRIMS" style={{height:"28px",objectFit:"contain"}}/></Link>
        <div style={{width:"60px"}}/>
      </header>

      {/* Hero — banner BELOW everything, text above */}
      <div style={{background:"#fff",borderBottom:"1.5px solid #E5E5E5"}}>
        {/* Banner — purely decorative at bottom of hero */}
        ...
        <div style={{maxWidth:"1100px",margin:"0 auto",padding:"0 1.25rem"}}>
          ...
          <div style={{display:"flex",alignItems:"flex-end",gap:"1.25rem",marginTop:"-48px",paddingBottom:"1.25rem",flexWrap:"wrap"}}>
            <div
              onClick={()=>dealer.logo&&setLightbox(dealer.logo)}
              style={{width:"96px",height:"96px",borderRadius:"14px",overflow:"hidden",border:"4px solid #fff",background:"#FFF7ED",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"var(--font-display)",fontSize:"2.5rem",color:"#F47B20",flexShrink:0,cursor:dealer.logo?"zoom-in":"default",boxShadow:"0 4px 20px rgba(0,0,0,0.15)",transition:"transform 0.2s"}}
            ...
            <div style={{flex:1,minWidth:"200px",paddingTop:"0.75rem",display:"flex",flexDirection:"column",gap:"0.35rem"}}>
              <h1 style={{fontFamily:"var(--font-display)",fontSize:"clamp(1.4rem,3vw,2.1rem)",letterSpacing:"0.04em",color:"#1A1A1A",lineHeight:1,margin:0}}>{dealer.companyName}</h1>
              {(dealer.city||dealer.state) && <div style={{fontSize:"0.875rem",color:"#737373"}}>📍 {[dealer.city,dealer.state].filter(Boolean).join(", ")}</div>}
              {dealer.description && <p style={{fontSize:"0.875rem",color:"#525252",lineHeight:1.6,maxWidth:"540px",margin:0}}>{dealer.description}</p>}
              <button onClick={toggleFollowers}
                ...
                <span style={{fontSize:"0.7rem",color:"#A3A3A3"}}>{showFollowers?"▲":"▼"}</span>
              </button>
            </div>

            {/* CTA buttons */}
            <div style={{display:"flex",gap:"0.625rem",flexWrap:"wrap",alignItems:"center",flexShrink:0,paddingTop:"0.75rem"}}>
              {!isOwn ? (
                <>
                  <FollowBtn dealerId={dealerId} onChange={setFollowerCount}/>
                  {isAuthenticated ? (
                    <button onClick={handleMessage} disabled={startMsg}
                      style={{background:"#1A1A1A",color:"#fff",border:"none",borderRadius:"8px",padding:"0.625rem 1.25rem",fontFamily:"var(--font-display)",fontSize:"0.82rem",letterSpacing:"0.08em",cursor:"pointer",opacity:startMsg?0.6:1,transition:"background 0.2s",whiteSpace:"nowrap"}}
                      onMouseOver={e=>(e.currentTarget.style.background="#F47B20")}
                      onMouseOut={e=>(e.currentTarget.style.background="#1A1A1A")}>
                      {startMsg?"Opening chat...":"💬 Message Dealer"}
                    </button>
                  ) : (
                    <Link href="/login" style={{background:"#1A1A1A",color:"#fff",borderRadius:"8px",padding:"0.625rem 1.25rem",fontFamily:"var(--font-display)",fontSize:"0.82rem",letterSpacing:"0.08em",textDecoration:"none",whiteSpace:"nowrap"}}>
                      Sign in to Message
                    </Link>
                  )}
                </>
          ...
          <div style={{display:"flex",gap:"0.5rem",flexWrap:"wrap",paddingBottom:"1.25rem",alignItems:"center"}}>
            {dealer.phone    && <a href={`tel:${dealer.phone}`} style={{background:"#EFF6FF",border:"1px solid #BFDBFE",color:"#1D4ED8",borderRadius:"8px",padding:"0.5rem 0.875rem",fontSize:"0.8rem",textDecoration:"none",fontWeight:600,whiteSpace:"nowrap"}}>📞 Call</a>}
            {dealer.whatsapp && <a href={`https://wa.me/${dealer.whatsapp.replace(/[^0-9]/g,"")}`} target="_blank" rel="noreferrer" style={{background:"#F0FDF4",border:"1px solid #86EFAC",color:"#15803D",borderRadius:"8px",padding:"0.5rem 0.875rem",fontSize:"0.8rem",textDecoration:"none",fontWeight:600,whiteSpace:"nowrap"}}>💬 WhatsApp</a>}
            {dealer.email    && <a href={`mailto:${dealer.email}`} style={{background:"#FFF7ED",border:"1px solid rgba(244,123,32,0.3)",color:"#C4621A",borderRadius:"8px",padding:"0.5rem 0.875rem",fontSize:"0.8rem",textDecoration:"none",fontWeight:600,whiteSpace:"nowrap"}}>✉ Email</a>}
            {socials.map(s => (
              <a key={s.label} href={s.url} target="_blank" rel="noreferrer"
                style={{background:"#F5F5F5",border:"1px solid #E5E5E5",color:"#525252",borderRadius:"8px",padding:"0.5rem 0.875rem",fontSize:"0.78rem",textDecoration:"none",fontWeight:500,whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:"0.35rem",transition:"all 0.15s"}}
      ...
      {/* Stats row */}
      <div style={{maxWidth:"1100px",margin:"1.25rem auto 0",padding:"0 1.25rem",display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:"0.875rem"}}>
        {[
          {label:"Vehicles Listed",value:dealer.totalCarsListed||dealer.availableCars?.length||0},
      ...
      <div style={{maxWidth:"1100px",margin:"1.25rem auto",padding:"0 1.25rem 3rem"}}>
        <h2 style={{fontFamily:"var(--font-display)",fontSize:"0.85rem",letterSpacing:"0.15em",color:"#737373",marginBottom:"1rem",textTransform:"uppercase" as const}}>
            ...
            {dealer.availableCars?.map((c:any)=>(
              <Link key={c._id||c.carId} href={`/cars/${c.carId}`}
                ...
                <div style={{padding:"0.875rem"}}>
                  ...
                  <div style={{fontFamily:"var(--font-display)",fontSize:"1.1rem",color:"#F47B20",marginTop:"0.375rem"}}>₦{(c.sellingPrice||0).toLocaleString()}</div>
                </div>
              </Link>
            ))}
          </div>
